...
import {
  Ban,
  Briefcase, CheckCheck, CheckCircle, ChevronLeft, ClipboardList, FileDown, FileText, Mail, Phone, RefreshCw, Stethoscope, User, UserX, X
...
import { NoShowAppointmentModal } from "@/components/appointments/NoShowAppointmentModal";
...
import { useCanCancelAnyAppointment, useCanCreateMedicalRecords, useCanManageAppointments, useCanUpdateAppointments } from "@/shared/permissions/capabilities";
...
export default function AdminAppointmentDetailScreen() {
  ...
  const canUpdate = useCanUpdateAppointments();
  ...
  const canCreateRecord = useCanCreateMedicalRecords();
  ...
  const handleConfirm = useCallback(async () => {
    if (!appointment) return;
    setConfirmMutating(true);
    try {
      const updated = await appointmentService.confirm(appointment.id);
      setCached(updated);
      updateLocal(updated);
    } catch (err) {
      throw err;
  ...
  const handleNoShow = useCallback(async () => {
    if (!appointment) return;
    const updated = await appointmentService.markNoShow(appointment.id);
    setCached(updated);
    updateLocal(updated);
  }, [appointment, updateLocal]);

  const handleMedicalRecord = useCallback(async () => {
    if (!appointment) return;
    setHcLoading(true);
    try {
      const existing = await medicalRecordService.getByAppointment(appointment.id);
      if (existing) {
        router.navigate({ pathname: "/admin/records/[id]", params: { id: existing.id } });
      } else {
        router.navigate({
          pathname: "/admin/records/create",
          params: { patientId: appointment.patientId, doctorId: appointment.doctorId, appointmentId: appointment.id },
        });
      }
    } catch {
      router.navigate({
    ...
    <View style={styles.root}>
      ...
      <SafeAreaView style={styles.container} edges={["top"]}>
        ...
        <View style={styles.headerRowLight}>
          ...
          <Text style={styles.headerTitleLight}>Administrar cita</Text>
          <TouchableOpacity
        ...
        >
          ...
          {canManage && (
            <AppointmentSection title="Paciente" icon={User}>
              <Text style={styles.personName}>
                {appointment.patient?.fullName ?? "Paciente"}
              </Text>
              {appointment.patient?.medicalId && (
                <Text style={styles.personDetail}>ID: {appointment.patient.medicalId}</Text>
              )}
              {appointment.patient?.email && (
                <View style={styles.contactRow}>
                  <Mail size={12} color="#0D9488" strokeWidth={2.2} />
                  <Text style={styles.contactText}>{appointment.patient.email}</Text>
                </View>
              )}
              {appointment.patient?.phone && (
                <View style={styles.contactRow}>
                  <Phone size={12} color="#0D9488" strokeWidth={2.2} />
                  <Text style={styles.contactText}>{appointment.patient.phone}</Text>
                </View>
              )}
            </AppointmentSection>
          )}
          ...
          {canCreateRecord && (
            <View style={styles.actionsBlock}>
              <TouchableOpacity
                style={[styles.actionButton, styles.actionHc]}
              ...
              >
                {hcLoading ? (
                  ...
                  <>
                    <ClipboardList size={16} color="#0D9488" strokeWidth={2.5} />
